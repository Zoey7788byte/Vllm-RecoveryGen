...
BASE_DIR="${BASE_DIR:-logs/RecoveryGen/Phase3_validation}"
SERVER_LOG_DIR="${SERVER_LOG_DIR:-${BASE_DIR}/server_logs/$(date +%Y%m%d_%H%M%S)}"
mkdir -p "${SERVER_LOG_DIR}"

# Recovery observability + Phase3 knobs
export VLLM_RECOVERY_OBS="${VLLM_RECOVERY_OBS:-1}"
export VLLM_RECOVERY_PHASE="${VLLM_RECOVERY_PHASE:-3}"
# Keep budgeted recovery path enabled for Phase3.
...
export REC_FALLBACK_PAUSE_DECODE="${REC_FALLBACK_PAUSE_DECODE:-1}"
...
CFG_TAG="${CFG_TAG:-phase3_m6_eval}"
...
for i in $(seq 1 600); do
  ...
  if [[ "${mcode}" == "200" && "${ccode}" == "200" ]]; then
    mkdir -p "$(dirname "${SERVER_SIG_FILE}")"
    {
      ...
      echo "  \"host\": \"${HOST}\","
      echo "  \"phase3_log_dir\": \"${SERVER_LOG_DIR}\","
      echo "  \"port\": ${PORT},"
      echo "  \"pid\": ${pid},"
      echo "  \"time\": \"$(date -Is)\""
      echo "}"
