...
OUT_BASE="${OUT_BASE:-logs/RecoveryGen/Phase1_validation/client_logs/${RUN_TAG}}"
mkdir -p "${OUT_BASE}"
...
SERVER_SIG_FILE="${SERVER_SIG_FILE:-logs/RecoveryGen/Phase1_validation/current_server_sig.json}"
...
# =========================
# schedule (old phase1 validation cadence)
# =========================
...
if [[ "${OUT_BASE}" == "logs/RecoveryGen/Phase1_validation/client_logs/${RUN_TAG}" ]]; then
...
PY
  ...
  OUT_BASE="logs/RecoveryGen/Phase1_validation/client_logs/${RUN_TAG}_${SERVER_SIG_SHORT}_${GATE_SIG}"
  mkdir -p "${OUT_BASE}"
fi
...
REPORT_JSON="${REPORT_JSON:-${OUT_BASE}/phase1_validation_report.json}"
REPORT_TXT="${REPORT_TXT:-${OUT_BASE}/phase1_validation_report.txt}"
...
write_progress "DONE" ""
python3 -u scripts/finalize_recovery_run_artifacts.py \
  --out-base "${OUT_BASE}" \
  --server-sig-file "${SERVER_SIG_FILE}" \
  --server-log-root "${SERVER_LOG_ROOT}"

echo "[OK] old phase1 cadence run done. summary=${SUMMARY_CSV}"

RECOVERY_DIR="${RECOVERY_DIR:-}"
if [[ -z "${RECOVERY_DIR}" && -s "${SERVER_SIG_FILE}" ]]; then
  RECOVERY_DIR="$(python3 - "${SERVER_SIG_FILE}" <<'PY'
import json
import os
import sys
...
cands = []
if isinstance(d.get("recovery_log_dir"), str):
    cands.append(d["recovery_log_dir"])
if isinstance(d.get("phase0_log_dir"), str):
    cands.append(os.path.join(d["phase0_log_dir"], "recovery_obs"))
    cands.append(os.path.join(d["phase0_log_dir"], "recovery"))
...
if [[ -n "${RECOVERY_DIR}" ]]; then
  EVENTS_PATH="${RECOVERY_DIR}/recovery_events.jsonl"
  TS_PATH="${RECOVERY_DIR}/recovery_ts.csv"
fi

python3 - "${SUMMARY_CSV}" "${EVENTS_PATH}" "${TS_PATH}" "${REPORT_JSON}" "${REPORT_TXT}" "${VLLM_RECOVERY_BUDGET:-16}" <<'PY'
import csv
import json
import math
import os
...
summary_csv, events_path, ts_path, report_json, report_txt, budget = sys.argv[1:]
budget = max(1, int(float(budget)))
...
preempt_sum = 0.0
stall_gap_max = float("nan")
for r in summary_rows:
    try:
        preempt_sum += float(r.get("preempt_sum_delta", "0") or 0)
    except Exception:
        pass
    try:
        v = float(r.get("stall_gap_max_s", "nan"))
        if math.isnan(stall_gap_max) or v > stall_gap_max:
            stall_gap_max = v
    except Exception:
        pass
...
swap_k_done = []
swap_by_seq = defaultdict(int)
remaining_by_seq = defaultdict(list)
commit_monotonic = True
commit_by_key = defaultdict(list)
...
if events_path and os.path.exists(events_path):
    with open(events_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if ev == "SWAP_IN_MICRO":
                try:
                    kd = float(detail.get("k_done", 0))
                    swap_k_done.append(kd)
                except Exception:
                    pass
                if seq is not None:
                    swap_by_seq[str(seq)] += 1
            elif ev == "RECOVERY_REMAINING_HOST_BLOCKS":
                if seq is not None:
                    try:
                        remaining_by_seq[str(seq)].append(float(detail.get("n_host", math.nan)))
                    except Exception:
                        pass
            elif ev == "RECOVERY_PROGRESS_COMMIT":
                kind = detail.get("kind", "unknown")
                rng = detail.get("range", [])
                if isinstance(rng, list) and len(rng) == 2:
                    try:
                        s = float(rng[0]); e2 = float(rng[1])
                    except Exception:
                        continue
                    key = (str(seq), str(kind))
                    commit_by_key[key].append((s, e2))

for _, arr in commit_by_key.items():
    max_end = -1e18
    for _, end in arr:
        if end + 1e-12 < max_end:
            commit_monotonic = False
            break
        if end > max_end:
            max_end = end
    if not commit_monotonic:
        break

seq_multi_micro = sum(1 for _, c in swap_by_seq.items() if c >= 2)
seq_stuck = 0
seq_to_zero = 0
for _, vals in remaining_by_seq.items():
    finite_vals = [v for v in vals if math.isfinite(v)]
    if not finite_vals:
        continue
    if max(finite_vals) > 0 and finite_vals[-1] <= 0:
        seq_to_zero += 1
    if finite_vals[-1] > 0:
        seq_stuck += 1

swapin_nonzero_count = 0
swapin_peak = float("nan")
swapin_mean_nonzero = float("nan")
if ts_path and os.path.exists(ts_path):
    ...
    with open(ts_path, "r", encoding="utf-8") as f:
        ...
        for r in rd:
            try:
                vals.append(float(r.get("swapin_blocks", "nan")))
            except Exception:
                pass
    nz = [v for v in vals if math.isfinite(v) and v > 0]
    swapin_nonzero_count = len(nz)
    if nz:
        swapin_peak = max(nz)
        swapin_mean_nonzero = statistics.mean(nz)

has_preempt = (preempt_sum > 0) or (event_counts.get("PREEMPT_TRIGGERED", 0) > 0)
micro_task_effective = bool(
    has_preempt
    and swap_k_done
    and max(swap_k_done) <= budget + 1e-9
    and seq_multi_micro >= 1
)
resumable_correct = bool((not has_preempt) or (seq_stuck == 0))
repeat_work_controlled = bool(commit_monotonic)
phase1_pass = micro_task_effective and resumable_correct and repeat_work_controlled

report = {
    "has_preempt": has_preempt,
    "preempt_sum_delta": preempt_sum,
    "stall_gap_max_s": stall_gap_max,
    "budget_blocks": budget,
    "events_path": events_path,
    "ts_path": ts_path,
    "event_counts": dict(event_counts),
    "swap_in_micro": {
        "count": len(swap_k_done),
        "k_done_max": (max(swap_k_done) if swap_k_done else None),
        "k_done_p50": (statistics.median(swap_k_done) if swap_k_done else None),
        "seq_with_multi_micro": seq_multi_micro,
    },
    "remaining_host": {
        "seq_to_zero": seq_to_zero,
        "seq_stuck_last_gt0": seq_stuck,
    },
    "commit_monotonic": commit_monotonic,
    "swapin_blocks_from_ts": {
        "nonzero_count": swapin_nonzero_count,
        "peak": swapin_peak,
        "mean_nonzero": swapin_mean_nonzero,
    },
    "criteria": {
        "micro_task_effective": micro_task_effective,
        "resumable_correct": resumable_correct,
        "repeat_work_controlled": repeat_work_controlled,
...
with open(report_txt, "w", encoding="utf-8") as f:
    f.write(f"phase1_pass={phase1_pass}\n")
    f.write(f"micro_task_effective={micro_task_effective}\n")
    f.write(f"resumable_correct={resumable_correct}\n")
    f.write(f"repeat_work_controlled={repeat_work_controlled}\n")
    f.write(f"preempt_sum_delta={preempt_sum:.6f}\n")
    f.write(f"swap_in_micro_count={len(swap_k_done)}\n")
    f.write(f"swap_in_k_done_max={(max(swap_k_done) if swap_k_done else 'nan')}\n")
    f.write(f"seq_with_multi_micro={seq_multi_micro}\n")
    f.write(f"seq_stuck_last_gt0={seq_stuck}\n")
    f.write(f"commit_monotonic={commit_monotonic}\n")
    f.write(f"events_path={events_path}\n")
    f.write(f"ts_path={ts_path}\n")
    f.write(f"summary_csv={summary_csv}\n")

print(f"[OK] wrote report json: {report_json}")
print(f"[OK] wrote report txt : {report_txt}")
print(f"[KEY] phase1_pass={phase1_pass} micro={micro_task_effective} resumable={resumable_correct} repeat={repeat_work_controlled}")
PY

echo "[OK] done. summary=${SUMMARY_CSV}"
echo "[OK] report_json=${REPORT_JSON}"
echo "[OK] report_txt=${REPORT_TXT}"
